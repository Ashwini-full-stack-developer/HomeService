...
public class surviceController : ControllerBase
    {
        ...
        public  IActionResult DeleteSurviceInformation([FromQuery]string id)
        {
            ...
            string query = "delete from labourregister03 where labourid = '" + id+ "'delete from survices where serviceid = '"+id+"'";
        ...
        {
            string sqlconnectionstring = "Data Source=LAPTOP-8HB43P71\\SQLEXPRESS;Initial Catalog=HomeSurviceFinalYearProject;Integrated Security=True;Encrypt=False";
            try
            {
                using (SqlConnection sqlconnecton = new SqlConnection(sqlconnectionstring))
                {
                    ...
                    string quare = "UPDATE [dbo].[Survices] " +
                                   "SET [ServiceName] = '" + survicemaster.ServiceName + "', " +
                                   "[ServiceImage] = '" + survicemaster.ServiceImage + "', " +
                                   "[ServiceCost] = '" + survicemaster.ServiceCost + "', " +
                                   "[ServiceDiscount] = '" + survicemaster.ServiceDiscount + "' " +
        ...
        {
            string conn = "Data Source=LAPTOP-8HB43P71\\SQLEXPRESS;Initial Catalog=HomeSurviceFinalYearProject;Integrated Security=True;Encrypt=False";
            string guid = Guid.NewGuid().ToString("N"); 
            string SurvicesID = guid.Substring(guid.Length - 4);
            string  labour = SurvicesID;

            string query = "INSERT INTO [dbo].[Survices] ([ServiceID],[ServiceName],[ServiceImage],[ServiceCost],[ServiceDiscount]) VALUES" + " ('" + SurvicesID + "' ,'" + newService.ServiceName + "' ,'" + newService.ServiceImage + "', " + newService.ServiceCost + "," + newService.ServiceDiscount + ")";
